...
// ESM (Node 18+). Make sure package.json has: "type": "module"
import { getStore } from "@netlify/blobs";

/** Allow your Webflow + Netlify origins */
const ALLOW_ORIGINS = [
...
/** Turn a data: URL into a public Netlify Blob URL (so DECOR8 can fetch it). */
async function toPublicUrlIfDataUrl(inputUrl){
  ...
  try {
    ...
    // Do not crash; return original data URL (some upstreams can ingest it) + note why
...
function devImageResponse(original, notes=[]){
  ...
  return { ok:true, info:{ upstream:false, images:[{ url: demo, width:1600, height:1067 }], echo:{ input_image_url: original }, notes } };
}
...
export const handler = async (event)=>{
  ...
  try {
    ...
    // Ensure DECOR8 receives a fetchable source URL
    const { url: preparedUrl, notes } = await toPublicUrlIfDataUrl(input_image_url);

    // Dev demo if asked or if upstream not configured yet
    if (forceDev || !env("STAGE_API_URL")) {
      return { statusCode:200, headers, body: JSON.stringify(devImageResponse(preparedUrl, notes)) };
    }

    // --- Upstream (DECOR8) ---
    const upstream = env("STAGE_API_URL"); // e.g. https://api.decor8.ai/v1/stage (set in Netlify env)
    const apiKey   = env("DECOR8_API_KEY") || env("STAGE_API_KEY") || env("OPENAI_API_KEY") || "";

    // Be generous with field names (some APIs want image_url, others input_image_url)
    const payload = {
      input_image_url: preparedUrl,
    ...
    try {
      ...
      return { statusCode:502, headers, body: JSON.stringify({ error:"Network to upstream failed", detail:String(netErr?.message||netErr), notes }) };
    }
    ...
    if (!images.length) {
      return { statusCode:502, headers, body: JSON.stringify({ error:"No images returned by upstream", detail:j, notes }) };
    }

    return { statusCode:200, headers, body: JSON.stringify({ ok:true, info:{ upstream:true, images, echo:{ input_image_url: preparedUrl }, notes } }) };
  } catch (err) {
